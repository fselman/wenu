...
class SequenceManifestFrame:
    """One deterministic expected output in a sequence manifest."""
...
class ObserverTimeSequenceManifest:
    """Portable deterministic identity for one observer-time frame plan."""
    ...
    def to_json(self) -> str:
        ...
        return _canonical_json(document, pretty=True) + "\n"
    ...
    def from_json(cls, text: str) -> "ObserverTimeSequenceManifest":
        ...
        expected_hash = document.pop("identity_sha256", None)
        try:
        ...
        if document != manifest._identity_payload():
...
def write_observer_time_sequence_manifest(
    ...
    """Atomically write the deterministic planned-sequence manifest."""
