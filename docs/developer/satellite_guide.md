...
## Lossless tabular views of an exact-crossing report
...
This separation is a reuse principle: scientific flattening and validation are
written once, so two formats cannot quietly acquire different meanings, and
later publication code can reuse the same in-memory representation. ECSV is a
...
satellite visible, illuminated, bright, or damaging to a detector.
