...
crossing solve, and returns no partial results. CLI and file behavior remain
...
two-call file workflow: an invalid JSON file solves no fields, writes one
separate validation-output JSON with ordered failures and a complete valid
subset, and requires explicit digest-bound revalidation on the second call.
