"""Reproducible 49H.2 real-render oracle audit-tool tests."""
...
from tools import render_49h2_fixed_sky_oracle as tool
...
def test_audit_records_manifest_times_dimensions_and_hashes(
    ...
    for index, color in enumerate(((10, 20, 30), (30, 20, 10))):
        path = oracle / f"frame-{index:04d}.png"
        Image.new("RGB", (4, 3), color).save(path)
        outputs.append(path)
    manifest = oracle / "wenu-sequence-manifest.json"
    manifest.write_text(
...
def test_audit_rejects_an_oracle_that_does_not_change(
    tmp_path,
    monkeypatch,
):
