...
def test_calendar_geometry_reserves_the_star_disk_and_strengthens_boundaries():
    ...
    for face in furniture.faces:
        assert face.star_disk_radius_mm == pytest.approx(88.0)
        assert face.outer_radius_mm == pytest.approx(100.0)
        for tick in face.ticks:
            assert np.hypot(*tick.inner) == pytest.approx(88.0)
            expected = 91.5 if tick.month_boundary else 90.0
        ...
        assert np.hypot(*face.day_labels[0].position) == pytest.approx(92.0)
        assert np.hypot(*face.month_labels[0].position) == pytest.approx(95.7)
