...
def test_50s6e_audits_multifov_interchange_and_lunar_illumination():
    ...
    for phrase in (
        ...
        "non-positive altitude fails admission",
        "defaults to 2",
        "exactly 30 degrees minimum altitude in this model",
        "Every point of the closed circular FoV, including its boundary",
        "exact spherical-footprint condition",
        "z_centre(t) + r_FoV <= arccos(1 / X_max)",
        "complete interval must be conservatively certified",
        "an oversized field, or uncertain numerical certification fails closed",
        "Arbitrary non- circular footprints remain later work",
