...
def test_point_layers_are_culled_to_viewport_before_renderer():
    observer = object()

    class PointsLayer(SkyLayer):
        layer_name = "viewport_points"

        def spherical_geometry(self, resolved_observer):
            assert resolved_observer is observer
            return SphericalPoints(
                lon_deg=[-10.0, 0.0, 10.0],
                lat_deg=[0.0, 0.0, 0.0],
                ids=["left", "center", "right"],
                metadata={"value": np.asarray([1, 2, 3])},
            )
    ...
    assert np.isnan(renderer.projected.x[[0, 2]]).all()
    assert renderer.projected.x[1] == pytest.approx(0.0)
    assert renderer.projected.ids.tolist() == ["left", "center", "right"]
    assert renderer.projected.metadata["value"].tolist() == [1, 2, 3]
    assert result.layers[0].projected is renderer.projected
